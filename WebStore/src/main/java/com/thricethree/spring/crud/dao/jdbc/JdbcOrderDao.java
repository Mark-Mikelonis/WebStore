...
public class JdbcOrderDao implements OrderDao{
	...
	private static final String SQL_FIND_CART_ID = 
			"SELECT * FROM orders WHERE user=? ORDER BY order_id DESC LIMIT 1;";
	...
	public Order getLastInsert(){
		Order lastInsert = null;
		
		Connection conn = null;
		
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(SQL_LAST_INSERT_ID);
			ResultSet rs = ps.executeQuery();
			if (rs.next()){
				lastInsert = new Order(
						rs.getInt("order_id"),
	...
	public int getCart(String user){
		...
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(SQL_FIND_CART_ID);
			ps.setString(1, user);
			ResultSet rs = ps.executeQuery();
			if (rs.next()){
				orderId = rs.getInt(1);
